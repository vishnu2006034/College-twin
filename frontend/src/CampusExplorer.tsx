...
import { buildings, floors, spaces, IT_BUILDING, kindLabels, linkedRoom } from './campus'
import './campus.css'
...
export default function CampusExplorer({ editionId, rooms }: { editionId: string; rooms: Room[] }) {
  ...
  function selectSpace(id: string) { const target = spaces.find(s => s.id === id); if (target) { setBuilding(IT_BUILDING); setFloor(target.floor); setSpace(id) } }
  function resetView() { setBuilding(null); setFloor(null); setSpace(null); setQuery(''); setReset(r => r + 1) }
  ...
  return <section className="campus-workspace" aria-label="Interactive campus twin">
    <div className="campus-heading"><div><p className="eyebrow">JEPPIAAR ENGINEERING COLLEGE</p><h1>Your campus, connected.</h1><p>Explore the campus. Step into a floor. Understand each space.</p></div><span className="model-badge"><i/> Spatial model · v1</span></div>
    <div className="explorer-grid">
      <aside className="campus-directory"><p className="eyebrow">EXPLORE SPACES</p><label htmlFor="campus-search">Find a building or room</label><input id="campus-search" type="search" placeholder="Search IT, F3, lab…" value={query} onChange={e => setQuery(e.target.value)}/>
        <div className="directory-list"><p className="directory-caption">CAMPUS BUILDINGS</p>{buildings.filter(b => `${b.label} ${b.detailed ? 'IT ECE' : ''}`.toLowerCase().includes(term)).map(b => <button key={b.id} className={`directory-item ${b.id === buildingId ? 'selected' : ''}`} aria-pressed={b.id === buildingId} onClick={() => selectBuilding(b.id)}><span>{b.label}</span><small>{b.detailed ? '3 floors + terrace ↗' : 'Exterior model'}</small></button>)}
        {term && <><p className="directory-caption">ROOMS & FACILITIES</p>{spaces.filter(s => `${s.label} ${s.department ?? ''} ${kindLabels[s.kind]}`.toLowerCase().includes(term)).map(s => <button className="directory-item" key={s.id} onClick={() => selectSpace(s.id)}><span>{s.label} · {kindLabels[s.kind]}</span><small>{floors[s.floor].label}</small></button>)}{!spaces.some(s => `${s.label} ${s.department ?? ''} ${kindLabels[s.kind]}`.toLowerCase().includes(term)) && <p>No matching rooms.</p>}</>}
      ...
      <div className="campus-main"><div className="scene-toolbar"><nav aria-label="Campus breadcrumbs"><button onClick={resetView}>Campus</button>{building && <><span>/</span><button onClick={() => selectBuilding(building.id)}>{building.label}</button></>}{floor !== null && <span>/ {floors[floor].label}</span>}</nav><button className="quiet" onClick={resetView}>Reset view</button></div>
        <div className="scene-viewport" role="region" aria-label="3D campus viewer"><SceneBoundary fallback={fallback}><Suspense fallback={<div className="scene-fallback" role="status">Loading campus geometry…</div>}>{webgl ? <CampusScene buildingId={buildingId} floor={floor} spaceId={spaceId} reset={reset} onBuilding={selectBuilding} onSpace={selectSpace} onFailure={() => setWebgl(false)}/> : fallback}</Suspense></SceneBoundary><div className="scene-caption">{floor === null ? 'CAMPUS MODEL' : floor === 3 ? 'OPEN TERRACE' : 'FLOOR CUTAWAY'}<span>Approximate geometry</span></div></div>
        ...
        {buildingId === IT_BUILDING && <section className="floor-explorer" aria-label="Building floors"><div className="floor-heading"><h2>Inside IT / ECE</h2><span>Garden-facing corridor</span></div><div className="floor-tabs">{floors.map(f => <button key={f.level} aria-pressed={floor === f.level} onClick={() => { setFloor(f.level); setSpace(null) }}>{f.label}</button>)}</div>
          {floor === null ? <p>Select a floor to reveal its rooms.</p> : floor === 3 ? <p>Open terrace with a parapet and access from the left and right stairs. Dimensions and access structures are approximate.</p> : <><div className="floor-orientation"><span>← Girls’ side · numbering begins here</span><span>Boys’ side →</span></div><div className="room-strip">{spaces.filter(s => s.floor === floor).sort((a, b) => a.x - b.x).map(s => <button key={s.id} aria-pressed={spaceId === s.id} className={`room-cell ${s.kind}`} onClick={() => selectSpace(s.id)}><strong>{s.label}</strong><small>{kindLabels[s.kind]}</small></button>)}</div><div className="corridor-strip">Single corridor · faces the central garden</div></>}
        </section>}
      </div>
      <aside className="space-inspector" aria-label="Selected space information" aria-live="polite"><p className="eyebrow">SPACE DETAILS</p>{space ? <><h2>{space.label}</h2><span className={`function-chip ${space.kind}`}>{kindLabels[space.kind]}</span><dl><dt>Building</dt><dd>IT / ECE · Block 3</dd><dt>Floor</dt><dd>{floors[space.floor].label}</dd><dt>Department</dt><dd>{space.department ?? 'Not confirmed'}</dd><dt>Assignment</dt><dd>{space.provisional ? 'Provisional' : 'User-confirmed'}</dd><dt>Geometry</dt><dd>Approximate</dd></dl><div className="data-link"><p className="eyebrow">DATA CONNECTION</p>{record ? <><p>Synthetic dataset record</p><strong>{record.code}</strong><p>Capacity: {record.capacity}</p>{record.lab_type && <p>Lab type: {record.lab_type}</p>}</> : <><h3>No linked dataset record</h3><p>This physical space has not been mapped to a room in the selected synthetic dataset.</p></>}</div></> : building ? <><h2>{building.label}</h2><p>{building.detailed ? 'Select a floor, then a room to inspect its function and data connection.' : 'Exterior geometry is available. Interior details have not been confirmed.'}</p><dl><dt>Reference</dt><dd>{building.confidence === 'provisional' ? 'Identity unconfirmed' : 'Campus imagery / user description'}</dd><dt>Dimensions</dt><dd>Approximate</dd></dl>{building.detailed && <button onClick={() => setFloor(1)}>Explore IT floor →</button>}</> : <><div className="inspector-symbol">⌘</div><h2>A place for every insight.</h2><p>Select a building to begin. IT / ECE includes explorable floors, classrooms, offices, labs, and facilities.</p><button onClick={() => { selectBuilding(IT_BUILDING); setFloor(1) }}>Explore IT floor →</button><div className="inspector-stat"><strong>3 + roof</strong><span>Detailed IT / ECE levels</span></div></>}
        <div className="inspector-disclaimer">Spatial reference model. Synthetic data remains separate. Live occupancy and simulations are not connected.</div>
      </aside>
    </div>
  </section>
}
