...
import { buildings, floorsFor, spaces, IT_BUILDING, HALL_BUILDING, kindLabels, linkedRoom } from './campus'
import './campus.css'
  ...
  return <section className="campus-workspace" aria-label="Interactive campus twin">
    ...
    <div className="explorer-grid">
      ...
      <div className="campus-main"><div className="scene-toolbar"><nav aria-label="Campus breadcrumbs"><button onClick={resetView}>Campus</button>{building && <><span>/</span><button onClick={() => selectBuilding(building.id)}>{building.label}</button></>}{floor !== null && <span>/ {floors[floor].label}</span>}</nav><button className="quiet" onClick={resetView}>Reset view</button></div>
        ...
        {building?.detailed && <section className="floor-explorer" aria-label="Building floors"><div className="floor-heading"><h2>Inside {building.label}</h2><span>{buildingId === HALL_BUILDING ? 'Open courtyard · garden at front' : 'Garden-facing corridor'}</span></div><div className="floor-tabs">{floors.map(f => <button key={f.level} aria-pressed={floor === f.level} onClick={() => { setFloor(f.level); setSpace(null) }}>{f.label}</button>)}</div>
          {floor === null ? <p>Select a floor to reveal its rooms.</p> : (buildingId === HALL_BUILDING ? floor === 2 : floor === 3) ? <p>{buildingId === HALL_BUILDING ? 'Upper terrace reached by the courtyard-side stairs (sketch marker 7). The central courtyard stays open; terrace dimensions are approximate.' : 'Open terrace with a parapet and access from the left and right stairs. Dimensions and access structures are approximate.'}</p> : buildingId === HALL_BUILDING ? <><p>Viewed from the garden: left is boys’ side; right is girls’ side. Door positions and dimensions are approximate.</p><div className="hall-plan" aria-label="Courtyard floor plan"><div className="hall-yard" style={{ top: '14%', height: '36%' }}>Open courtyard</div>{floorSpaces.map(s => <button key={s.id} aria-pressed={spaceId === s.id} style={{ left: `${(s.x - s.width / 2 + 32.5) / 65 * 100}%`, top: `${(s.z! - s.depth! / 2 + 32.5) / 65 * 100}%`, width: `${s.width / 65 * 100}%`, height: `${s.depth! / 65 * 100}%` }} onClick={() => selectSpace(s.id)}>{s.label}</button>)}</div><div className="corridor-strip">Garden side · {floor === 0 ? 'Single main entrance → passage → courtyard' : 'Front corridor → auditorium · open courtyard behind auditorium'}</div></> : <>{floors[floor].label.includes('Unconfirmed') && <p role="status">Second-floor use and rooms are unconfirmed. Only provisional stairs and restrooms are shown.</p>}<div className="floor-orientation"><span>← {leftRestroom} side · numbering begins here</span><span>{rightRestroom} side →</span></div><div className="room-strip">{floorSpaces.map(s => <button key={s.id} aria-pressed={spaceId === s.id} className={`room-cell ${s.kind}`} onClick={() => selectSpace(s.id)}><strong>{s.label}</strong><small>{kindLabels[s.kind]}</small></button>)}</div><div className="corridor-strip">Single corridor · faces the central garden</div></>}
        </section>}
      </div>
      <aside className="space-inspector" aria-label="Selected space information" aria-live="polite"><p className="eyebrow">SPACE DETAILS</p>{space ? <><h2>{space.label}</h2><span className={`function-chip ${space.kind}`}>{kindLabels[space.kind]}</span><dl><dt>Building</dt><dd>{building?.label}</dd><dt>Floor</dt><dd>{floors[space.floor].label}</dd><dt>Department</dt><dd>{space.department ?? 'Not confirmed'}</dd><dt>Assignment</dt><dd>{space.provisional ? 'Provisional' : 'User-confirmed'}</dd><dt>Geometry</dt><dd>Approximate</dd></dl>{space.note && <p>{space.note}</p>}<div className="data-link"><p className="eyebrow">DATA CONNECTION</p>{record ? <><p>Synthetic dataset record</p><strong>{record.code}</strong><p>Capacity: {record.capacity}</p>{record.lab_type && <p>Lab type: {record.lab_type}</p>}</> : <><h3>No linked dataset record</h3><p>This physical space has not been mapped to a room in the selected synthetic dataset.</p></>}</div></> : building ? <><h2>{building.label}</h2><p>{building.detailed ? 'Select a floor, then a room to inspect its function and data connection.' : 'Exterior geometry is available. Interior details have not been confirmed.'}</p><dl><dt>Reference</dt><dd>{building.confidence === 'provisional' ? 'Identity unconfirmed' : 'Campus imagery / user description'}</dd><dt>Dimensions</dt><dd>Approximate</dd></dl>{building.detailed && <button onClick={() => { setFloor(1); setSpace(null) }}>Explore {buildingId === IT_BUILDING ? 'IT' : building.label} floor →</button>}</> : <><div className="inspector-symbol">⌘</div><h2>A place for every insight.</h2><p>Select a building to begin. IT / ECE, CSE, ECE and the lab building include explorable floors, classrooms, offices, labs, and facilities.</p><button onClick={() => { selectBuilding(IT_BUILDING); setFloor(1) }}>Explore IT floor →</button><div className="inspector-stat"><strong>Floor by floor</strong><span>Explore confirmed uses and provisional geometry</span></div></>}
